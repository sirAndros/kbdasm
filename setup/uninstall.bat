@echo off
if not exist %windir%\system32\kbdusru.dll goto:notexist
echo To uninstall, you must run this command with administrative privilegies.
set /p answer="Do you want to uninstall kbdusru.dll from your system? [Y/N] "
if "%answer%"=="" goto:no
...
goto:eof
:yes
del %windir%\system32\kbdusru.dll >nul 2>&1
if exist %windir%\system32\kbdusru.dll goto:cannotdel
...
goto:eof
:notexist
echo There is no kbdusru.dll installed.
goto:eof
:cannotdel
